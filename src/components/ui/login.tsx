...
>(({ material, width, count, height }, ref) => {
  ...
  // Static by default — only advance the shader time uniform when the cursor
  // moves, scaled by how far it moved. The decay ref smooths it so the beams
  // ease back to a halt after the pointer stops.
  const mouseEnergy = useRef(0)
  ...
  useEffect(() => {
    ...
    const onMove = (e: MouseEvent) => {
      if (seeded) {
        ...
        mouseEnergy.current += Math.sqrt(dx * dx + dy * dy) * 0.0015
        if (mouseEnergy.current > 0.4) mouseEnergy.current = 0.4
      }
      lastX = e.clientX
      lastY = e.clientY
      seeded = true
  ...
  useFrame((_, delta) => {
    if (mouseEnergy.current > 0.0001) {
      mesh.current.material.uniforms.time.value += mouseEnergy.current * delta * 6
      mouseEnergy.current *= 0.92
    }
  })
